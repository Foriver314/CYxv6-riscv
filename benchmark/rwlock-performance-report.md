...
当前实现没有把文件系统 inode/buffer 锁替换为读写锁。本报告使用 `user/rwbench.c` 微基准隔离测量读写锁自身相对“全部操作都用写锁串行化”的收益和代价。
...
因此，读写锁适合用于 xv6 中“读路径频繁、写路径较少”的共享数据结构；对于读少写多或写路径占主导的结构，继续使用普通互斥/自旋/睡眠锁通常更简单，也不会损失可观性能。
